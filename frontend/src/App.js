...
const mockCryptoData = [
  {
    ...
    current_price: 61000.5,
    price_change_percentage_24h: 1.25,
    market_cap: 1200000000000,
    sparkline_in_7d: {
      price: [60000, 61500, 61000, 62000, 61800, 62500, 61000.5],
    },
  },
  {
    id: "ethereum",
    ...
    market_cap: 408000000000,
    sparkline_in_7d: { price: [3450, 3380, 3420, 3350, 3410, 3390, 3400.75] },
  },
  {
    id: "solana",
    symbol: "sol",
    ...
    sparkline_in_7d: { price: [128, 130, 132, 129, 135, 138, 135.2] },
  },
  {
    id: "cardano",
    symbol: "ada",
    ...
    sparkline_in_7d: { price: [0.39, 0.385, 0.392, 0.381, 0.375, 0.385, 0.38] },
  },
  {
    id: "dogecoin",
    symbol: "doge",
    ...
    sparkline_in_7d: {
      price: [0.115, 0.118, 0.122, 0.119, 0.125, 0.123, 0.12],
    },
  },
];
...
const Header = ({ theme, toggleTheme, searchTerm, setSearchTerm }) => (
  <header className="p-4 flex justify-between items-center bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-20">
    ...
    <div className="flex items-center gap-4">
      ...
      <a
        href="https://github.com/ArturRSA19/Crypto-Dashboard" // Placeholder link
        target="_blank"
  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-2 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-lg">
        <p className="font-bold text-gray-900 dark:text-white">{`Price: ${formatCurrency(
          payload[0].value
        )}`}</p>
    ...
    <div className="grid grid-cols-3 md:grid-cols-5 items-center p-4 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-200">
      <div className="col-span-1 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 uppercase">
            {coin.symbol}
          </p>
        </div>
      </div>
      ...
      <div
        className={`col-span-1 flex items-center justify-end md:justify-start ${
      ...
      >
        ...
        <span className="font-semibold text-sm md:text-base ml-1">
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-md flex flex-col h-[600px]">
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 p-4 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-10">
        <p className="col-span-1 font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          Coin
        </p>
        <p className="col-span-1 font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right md:text-left">
          Price
        </p>
        <p className="col-span-1 font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider text-right md:text-left">
          24h Change
        </p>
        <p className="hidden md:block col-span-1 font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          Market Cap
        </p>
        <p className="hidden md:block col-span-1 font-bold text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
          Last 7 Days
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col">
        ...
        <footer className="p-4 border-t border-gray-200 dark:border-gray-800 text-center">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Generated by Gemini. Not financial advice. Always do your own
            research.
          </p>
        </footer>
      </div>
...
const MainChart = ({ coinData, selectedCoin, setSelectedCoin }) => {
  ...
  const chartData = useMemo(() => {
    if (
      !activeCoin ||
      !activeCoin.sparkline_in_7d ||
      !activeCoin.sparkline_in_7d.price
    ) {
      return [];
    }
  ...
  const fillGradient = isPositive
    ? "url(#chartPositiveGradient)"
    : "url(#chartNegativeGradient)";
    ...
    <>
      ...
      <div className="p-4 md:p-6 bg-white dark:bg-gray-900 rounded-lg shadow-md">
        ...
        <div className="flex justify-center">
          <button
            onClick={handleAnalyzeCoin}
            disabled={isAnalyzing}
            className="flex items-center gap-2 px-6 py-3 font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-900 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span>
              {isAnalyzing
                ? "Analyzing..."
                : `✨ AI Analysis for ${activeCoin.name}`}
            </span>
          </button>
        </div>
      </div>
  ...
  useEffect(() => {
    const fetchCoins = async () => {
      ...
      try {
        ...
        setError("Failed to load data. Displaying cached results.");
        setCoins(mockCryptoData);
      } finally {
        setLoading(false);
      }
