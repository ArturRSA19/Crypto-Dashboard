            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 overflow-y-auto">
                 ...
                 <footer className="p-4 border-t border-gray-200 dark:border-gray-800 text-center">
                    <p className="text-xs text-gray-500 dark:text-gray-400">Generated by OpenAI. Not financial advice. Always do your own research.</p>
                </footer>
            </div>
        </div>
    );
        ...
        try {
            const response = await fetch('http://localhost:5000/api/analyze', {
              ...
              body: JSON.stringify({
                ...
                currentPrice: activeCoin.current_price,
                change24h: activeCoin.price_change_percentage_24h,
              })
            });
            ...
            if (!response.ok) {
              throw new Error(result?.message || `Backend analysis failed with status: ${response.status}`);
            }
            ...
            if (message.includes('429') || message.includes('rate limit')) {
              setAnalysis('The AI service is currently rate-limited or out of quota (HTTP 429). Please wait a few minutes and try again.');
            } else if (message.includes('openai api key') || message.includes('openai_api_key')) {
              setAnalysis('OpenAI key is missing on backend. Add OPENAI_API_KEY in backend/.env and restart the backend server.');
            } else {
